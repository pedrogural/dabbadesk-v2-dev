        <div x-show="tab === 'finance'" x-cloak class="space-y-5">
            ...
            <section class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <span class="rounded-full {{ $reversiblePaymentCount > 0 ? 'bg-rose-50 text-rose-700 ring-rose-200' : 'bg-slate-100 text-slate-500 ring-slate-200' }} px-3 py-1 text-xs font-black uppercase tracking-wide ring-1">
                        {{ $reversiblePaymentCount }} reversible
                    </span>
                </div>
                    ...
                    <div class="divide-y divide-slate-200">
                        @forelse ($paymentRows as $paymentRow)
                            ...
                            <div class="grid gap-3 px-4 py-3 text-sm md:grid-cols-[1fr_1fr_1fr_1fr_auto] md:items-center">
                                <div>
                                    <p class="font-black text-slate-900">{{ $paymentDate }}</p>
                                    <p class="mt-0.5 text-[11px] font-semibold text-slate-400 md:hidden">Date</p>
                                </div>
                                <div>
                                    <p class="font-bold text-slate-700">{{ $paymentMethodLabel }}</p>
                                    @if ($paymentRow->provider)
                                        <p class="mt-0.5 text-[11px] font-semibold text-slate-400">{{ $paymentRow->provider }}</p>
                                    @endif
                                </div>
                                <div class="min-w-0">
                                    <p class="truncate font-semibold text-slate-600">{{ $paymentReferenceLabel }}</p>
                                    @if ($paymentRow->note)
                                        <p class="mt-0.5 line-clamp-1 text-[11px] font-semibold text-slate-400">{{ $paymentRow->note }}</p>
                                    @endif
                                </div>
                                <div class="md:text-right">
                                    <p class="font-black text-emerald-700">£{{ number_format($paymentAmount, 2) }}</p>
