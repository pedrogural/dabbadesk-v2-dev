...
<div class="grid gap-4 lg:grid-cols-2">
    <form method="POST" action="{{ route('purchasing.purchases.store') }}" class="rounded-2xl border border-emerald-100 bg-emerald-50/70 p-4">
        @csrf
        ...
        <div class="flex items-start justify-between gap-3">
            ...
            <span class="rounded-full bg-white px-2.5 py-1 text-xs font-black text-emerald-700 ring-1 ring-emerald-100">{{ $remainingToBuy }} left</span>
        </div>

        <div class="mt-3 grid gap-2 sm:grid-cols-2">
            <label class="text-xs font-bold text-slate-600">Qty
                <input name="qty" type="number" min="1" max="{{ max(1, $remainingToBuy) }}" value="{{ max(1, $remainingToBuy) }}" class="mt-1 h-10 w-full rounded-xl border-slate-300 text-sm font-bold" @disabled(! $canRecordPurchase)>
            </label>
            <label class="text-xs font-bold text-slate-600">Unit cost
                <input name="purchase_unit_price" type="number" step="0.01" min="0" placeholder="0.00" class="mt-1 h-10 w-full rounded-xl border-slate-300 text-sm font-bold" @disabled(! $canRecordPurchase)>
            </label>
            <label class="sm:col-span-2 text-xs font-bold text-slate-600">Retailer order reference
                <input name="retailer_order_reference" type="text" placeholder="e.g. 204-1234567-1234567" class="mt-1 h-10 w-full rounded-xl border-slate-300 text-sm font-bold" @disabled(! $canRecordPurchase)>
            </label>
            <label class="text-xs font-bold text-slate-600">Marketplace seller
                <input name="marketplace_seller" type="text" placeholder="Optional" class="mt-1 h-10 w-full rounded-xl border-slate-300 text-sm font-bold" @disabled(! $canRecordPurchase)>
            </label>
            <label class="text-xs font-bold text-slate-600">Ordered date
                <input name="ordered_at" type="date" value="{{ now()->toDateString() }}" class="mt-1 h-10 w-full rounded-xl border-slate-300 text-sm font-bold" @disabled(! $canRecordPurchase)>
            </label>
            <label class="text-xs font-bold text-slate-600">Expected dispatch
                <input name="expected_dispatch_at" type="date" class="mt-1 h-10 w-full rounded-xl border-slate-300 text-sm font-bold" @disabled(! $canRecordPurchase)>
            </label>
            <label class="text-xs font-bold text-slate-600">Expected UK hub
                <input name="expected_uk_hub_at" type="date" class="mt-1 h-10 w-full rounded-xl border-slate-300 text-sm font-bold" @disabled(! $canRecordPurchase)>
            </label>
            <label class="sm:col-span-2 text-xs font-bold text-slate-600">Purchase note
                <textarea name="note" rows="2" placeholder="Optional note" class="mt-1 w-full rounded-xl border-slate-300 text-sm font-bold" @disabled(! $canRecordPurchase)></textarea>
            </label>
            <label class="sm:col-span-2 flex items-center gap-2 text-xs font-bold text-slate-600">
                <input name="requires_marking_attention" value="1" type="checkbox" class="rounded border-slate-300 text-emerald-600" @disabled(! $canRecordPurchase)>
                Needs marking attention on arrival
            </label>
        </div>

        <button class="mt-3 w-full rounded-xl bg-emerald-600 px-3 py-2 text-xs font-black text-white shadow-sm hover:bg-emerald-700 disabled:bg-slate-300" @disabled(! $canRecordPurchase)>Save purchase</button>
    </form>

    <form method="POST" action="{{ route('purchasing.problems.store') }}" class="rounded-2xl border border-rose-100 bg-rose-50/70 p-4">
        @csrf
        <input type="hidden" name="order_item_id" value="{{ $item->id }}">
        <div class="flex items-start justify-between gap-3">
            <div>
            ...
            <span class="rounded-full bg-white px-2.5 py-1 text-xs font-black text-rose-700 ring-1 ring-rose-100">Qty {{ $problemActionQty }}</span>
        </div>

        <div class="mt-3 grid gap-2 sm:grid-cols-2">
            <label class="text-xs font-bold text-slate-600">Qty
                <input name="qty" type="number" min="1" value="{{ $problemActionQty }}" class="mt-1 h-10 w-full rounded-xl border-slate-300 text-sm font-bold">
            </label>
