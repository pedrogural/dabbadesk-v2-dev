<x-app-layout>
    ...
    <div
        ...
        x-data="{
            ...
            customerError: '',
            customerTimer: null,
            customerSearchUrl: '{{ route('money-desk.customer-search') }}',
            ...
            orderError: '',
            orderTimer: null,
            orderSearchUrl: '{{ route('money-desk.order-search') }}',
        ...
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
            <div class="rounded-2xl bg-white p-6 shadow-sm border border-slate-200">
                ...
                <p class="mt-3 text-3xl font-bold text-slate-900">
                    £{{ number_format($stats['payments_received'] ?? 0, 2) }}
                </p>
            ...
            <div class="rounded-2xl bg-white p-6 shadow-sm border border-slate-200">
                ...
                <p class="mt-3 text-3xl font-bold text-emerald-600">
                    £{{ number_format($stats['orders_settled'] ?? 0, 2) }}
                </p>
            ...
            <div class="rounded-2xl bg-white p-6 shadow-sm border border-slate-200">
                ...
                <p class="mt-3 text-3xl font-bold text-indigo-600">
                    £{{ number_format($stats['wallet_balance'] ?? 0, 2) }}
                </p>
            ...
            <div class="rounded-2xl bg-white p-6 shadow-sm border border-slate-200">
                ...
                <p class="mt-3 text-3xl font-bold text-rose-600">
                    £{{ number_format($stats['refunds'] ?? 0, 2) }}
                </p>
                            ...
                            @forelse ($recentLedgerEntries as $entry)
                                <tr>
                                    ...
                                    <td class="py-3 pr-4 text-right font-semibold text-slate-900">
                                        {{ $entry->currency ?? 'GBP' }} {{ number_format($entry->amount, 2) }}
                                    </td>
                                </tr>
                            @empty
                                <tr>
            ...
            <div x-show="tab === 'orders'" x-transition class="mt-6">
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse ($recentOrderTransactions as $transaction)
                                <tr>
                                    <td class="py-3 pr-4 text-slate-500">
                                    ...
                                    <td class="py-3 pr-4 text-right font-semibold text-slate-900">
                                        {{ $transaction->currency ?? 'GBP' }} {{ number_format($transaction->amount, 2) }}
                                    </td>
                                </tr>
                            @empty
                                <tr>
            ...
            <div x-show="tab === 'wallet'" x-transition class="mt-6">
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse ($openWalletCredits as $credit)
                                <tr>
                                    <td class="py-3 pr-4 text-slate-500">
                                        {{ $credit->created_at ? \Carbon\Carbon::parse($credit->created_at)->format('d M Y') : '—' }}
                                    </td>
                                    ...
                                    <td class="py-3 pr-4 text-right text-slate-600">
                                        {{ $credit->currency ?? 'GBP' }} {{ number_format($credit->amount, 2) }}
                                    </td>

                                    <td class="py-3 pr-4 text-right font-bold text-purple-600">
                                        {{ $credit->currency ?? 'GBP' }} {{ number_format($credit->remaining_amount, 2) }}
                                    </td>
                                </tr>
                            @empty
                                <tr>
