            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">
                        Edit converted requests before they become locked operational order snapshots.
                    </p>
                </div>
                <span class="inline-flex w-fit rounded-full bg-indigo-100 px-4 py-2 text-sm font-semibold text-indigo-700">
                    Request → Draft → Order
                </span>
            </div>
        </div>
        ...
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <form method="GET" action="{{ route('draft-orders.index') }}" class="grid grid-cols-1 gap-4 lg:grid-cols-12 lg:items-end" x-data="{ timer: null, submitSoon() { clearTimeout(this.timer); this.timer = setTimeout(() => this.$refs.form.submit(), 450); } }" x-ref="form">
                <div class="lg:col-span-7">
                    ...
                    <input id="q" name="q" value="{{ $filters['q'] }}" type="text" placeholder="Draft number, request ref, customer, item, SKU or URL" class="mt-2 w-full rounded-2xl border-slate-300 px-4 py-3 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" @input="submitSoon()">
                </div>
                        ...
                        <div class="grid grid-cols-1 gap-4 xl:grid-cols-12 xl:items-center">
                            <div class="xl:col-span-4">
                                <div class="flex flex-wrap items-center gap-2">
                                    <h3 class="text-lg font-bold text-slate-900">#{{ $draft->draft_number ?: $draft->id }}</h3>
