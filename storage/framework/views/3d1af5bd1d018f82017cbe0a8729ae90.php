...
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
    ...
    <?php
        ...
        $copyFullAddress = $addressLines->implode("\n");
        ...
        $isHistoricalRevision = $revisionState === 'superseded';
        $revisionRows = collect($revisionHistory ?? []);
        $activeRevision = $revisionRows
            ->first(fn ($revision) => in_array((string) ($revision->revision_state ?? ''), ['current', 'current_revision'], true))
            ?: $revisionRows->last();
        $activeRevisionUrl = $activeRevision ? route('orders.show', $activeRevision->id) : route('orders.index');
